...
sorted_search_dict = dict(sorted(search_dict.items(), key=lambda item: item[1][-1]))
print(f"The first 4 cars (print up to five) is not older {search_criteria[0]}, with engine_volume more then {search_criteria[1]} "
      f"and with the price less then {search_criteria[2]}: ")
for key in sorted_search_dict.keys():
    if count < 4:
        print("   ", sorted_search_dict.get(key))
        count += 1
